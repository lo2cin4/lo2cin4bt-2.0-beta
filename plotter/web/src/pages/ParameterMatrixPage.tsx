    ...
    <div className="page-stack">
            ...
            return (
              <div key={key} className={`candidate-card ${row.backtest_id === highlightedBacktestId ? 'candidate-card-selected' : ''}`}>
                ...
                <div className="candidate-card-copy">
                  <div className="candidate-card-title" data-private-strategy="identity">{row.label}</div>
      ...
      <SectionCard title={t('parameterMatrix.candidateEvidence')} subtitle={pm('說明這些參數組合為何在執行前向分析 (WFA) 前被保留檢視。', 'Explains why these parameter combinations were kept for review before Walk-Forward Analysis (WFA).')}>
        <div className="research-diagnostics-grid">
          <div className="research-diagnostics-card"><div className="research-diagnostics-title">{pm('最高排名摘要', 'Top-Ranked Summary')}</div><div className="research-diagnostics-hero" data-private-strategy="identity">{topTrial?.label || '-'}</div><div className="research-diagnostics-copy" data-private-strategy="rules">{topTrial ? formatParams(topTrial.params) : pm('沒有可用候選組合。', 'No candidate is available.')}</div><div className="research-diagnostics-meta">{pm('穩健', 'Robust')} {formatMetric(topTrial?.robust_score)} | OOS {formatMetric(topTrial?.mean_oos_sharpe)}</div></div>
          <div className="research-diagnostics-card"><div className="research-diagnostics-title">{pm('參數重要度', 'Parameter Importance')} <InfoHint label={pm('參數重要度', 'Parameter Importance')} body={pm('用來粗略顯示哪些參數較能解釋目前候選結果的差異。數值越高代表該參數在這批回測結果中的影響較明顯；它不是單獨的買賣理由，也不是 WFA 通過證據。', 'A rough read of which parameters explain more of the variation in the current candidate results. Higher values mean the parameter mattered more inside this batch; it is not a standalone trading reason or WFA pass evidence.')} /></div>{(payload.parameter_importance || []).slice(0, 5).map((item) => <div key={item.parameter} className="research-diagnostics-row" data-private-strategy="rules"><span>{axisLabel(item.parameter)}</span><strong>{formatMetric(item.importance, 3)}</strong></div>)}</div>
