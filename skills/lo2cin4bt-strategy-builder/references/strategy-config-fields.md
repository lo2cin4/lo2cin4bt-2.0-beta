...
| `fill_model` | fill timing, costs, slippage, accounting backend | omitting cost model |
...
- Do not create one `single_backtest` file per parameter combination. For example, threshold `10, 11, 12, 13, 14, 15` and holding days `50, 100, 150, 200, 250` is one matrix config with 30 candidate combinations, not 30 separate strategies.
