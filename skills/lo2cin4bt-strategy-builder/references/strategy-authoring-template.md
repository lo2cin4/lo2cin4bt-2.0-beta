...
Use this reference when a user describes a strategy in plain language and asks lo2cin4bt or `lo2cin4` to build it.
...
For any local file, external event table, custom universe, custom benchmark, factor, or feature contract, require per-feature `data_availability`:
...
If these are missing, `workspace_doctor` must fail the contract. If the author cannot answer them, ask the user instead of writing a runnable config.
